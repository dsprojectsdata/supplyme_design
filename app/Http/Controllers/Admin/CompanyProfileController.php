...
class CompanyProfileController extends Controller
{
  ...
  {
    $user_id = Auth::guard('web')->user()->id;
    $user_campany_id = Auth::user()->company_id;
    $company = Company::where('id',$user_campany_id)->first();
    $ompanyProfile = CompanyProfile::where('user_id', $user_id)->first();
    $companyPrimaryContacts = CompanyPrimaryContact::where('user_id', $user_id)->get();
    ...
    return view('Admin.company-profile.company-index', compact('companyPrimaryContacts','companyCurrency', 'companyTypes', 'companyProfilePositions', 'companyIndustry', 'companyCertification', 'subcategory', 'category', 'countries', 'ndas', 'typesOfCompany', 'company', 'ompanyProfile', 'brandWebsitesLogo', 'companyCustomerClient', 'companyRegAddress', 'companyProductService','Jobrole'));
  }
  ...
  public function companyPrimary($id)
  {
    Log::info('hit on fuction delelteCompanyFinancialImage' . $id);
    $companyPrimary = CompanyPrimaryContact::find($id);
    $companyPrimary->delete();
    $values = [
      'success' => true,
      'message' => 'Deleted successfully .',
      'data' => $companyPrimary,
